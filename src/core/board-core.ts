...
export class BoardComponentImpl {
  events: BoardEvents;
  ...
  constructor(canvas: HTMLCanvasElement, events: BoardEvents) {
    this.canvas = canvas;
  ...
  static async of(
    canvas: HTMLCanvasElement,
    events: BoardEvents,
  ): Promise<BoardComponentImpl> {
    const board = new BoardComponentImpl(canvas, events);
    await board.connectedCallback();
    return board;
  }
