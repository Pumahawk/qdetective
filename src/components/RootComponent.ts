...
export function RootComponentF(cr: CustomElementRegistry) {
  class RootComponent extends HTMLElement implements RootComponent {
    ...
    connectedCallback() {
      ...
      const parameters = new URL(globalThis.window.location.href);
      const server = parameters.searchParams.get("server");
      const gameId = parameters.searchParams.get("gameId");
      if (server && gameId) {
